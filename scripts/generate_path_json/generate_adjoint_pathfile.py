...
adjoint_base = os.path.join(superbase, "adjoint")
...
def generate_json_dirfiles(eventname):
    ...
    for period in period_list:
        ...
        parlist['window_file'] = \
            os.path.join(window_base, "%s.%s" % (eventname, period),
                         "windows.json")
        parlist['figure_mode'] = False
        parlist['figure_dir'] = "/path/to/figure_dir"
        parlist['output_file'] = os.path.join(adjoint_base, "%s.%s.adjoint.h5"
                                              % (event, period))
        par_jsonfile = os.path.join(output_json_dir, "%s.%s.adjoint.path.json"
                                    % (event, period))
